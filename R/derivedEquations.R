  ...
  if (!is.null(events)) {
    ...
    events.addon <- lapply(1:nrow(events), function(i) {
      ...
      if (myevent[["method"]] == "replace") {
        ...
        vars <- intersect(paste0(xone, ".", rootpar), newvariables)
        if (length(vars) > 0) {
          d[[2]] <- data.frame(
            var = vars,
            time = tau,
            value = paste0("((", J11, ")*(", xone, "- (", xi, ")) - (", f1, "))/(", fr, ")"),
            root = root,
            method = "add"
          )
        }
        
        vars <- intersect(paste0(xk, ".", rootpar), newvariables)
        if (length(vars) > 0) {
          d[[3]] <- data.frame(
            var = vars,
            time = tau,
            value = paste0("((", Jk1, ")*(", xone, "- (", xi, ")))/(", fr, ")"),
            root = root,
            method = "add"
          )
        }
      ...
      # generate additional events for **multiplicative event**
        ...
        if (length(vars) > 0) {
          d[[2]] <- data.frame(
            var = vars,
            time = tau,
            value = paste0("((1 - (", xi, "))*((", J11, ")*(", xone, ") - (", f1, ")))/(", fr, ")"),
            root = root,
            method = "add"
          )
        }
