  ...
  if (!is.null(events)) {
    events.addon <- do.call(rbind, lapply(1:nrow(events), function(i) {
      ...
      if (length(tvar) > 0) {
        ...
        df_Var <- data.frame(
          var   = paste(var, tvar, sep = "."), # var is definitely non-zero, thus it doesn't need to be captured
          time  = events[["time"]][i],
          value = switch(
            as.character(events[["method"]][i]),
